...
public class GameOptionPanel extends JPanel {
	...
	private ButtonGroup buttonGroup = new ButtonGroup();  //相斥的按键组
	private ImageIcon ico = new ImageIcon(GameOptionPanel.class.getResource("/ico.png"));//查找带有给定名称的资源，委托给此对象的类加载器
	...
	private final JCheckBox checkBox_drawGridding = new JCheckBox();//Map选择
	private final JRadioButton radioButton_map1;////圆点选项
	...
	public GameOptionPanel() {
		super();
		setSize(450, 185);
		setLayout(null);//流式布局，null清空布局管理器
		setBorder(new EtchedBorder(EtchedBorder.LOWERED));//设置边框，蚀刻边框
		setFocusable(false);

		final JSeparator separator = new JSeparator();//用作菜单项的分隔符
		...
		separator.add(checkBox_drawGridding);//“显示网格”复选框
		...
		separator_6.add(button_backgroundColor);//“设置背景”框体
		...
		separator_4.add(button_foodColor);//"设置食物颜色"框体
		...
		separator_4.add(radioButton_map1);//"地图1"圆点选项
		...
		separator_4.add(radioButton_map2);//"地图2"圆点选项
		...
		separator_1.add(button_headColor);//"设置蛇头颜色"框体
		...
		separator_1.add(button_bodyColor);//"设置蛇身颜色"框体
		...
		separator_5.add(newGameButton);//"开始新游戏"框体
		newGameButton.setFocusable(false);
		newGameButton.setText("开始新游戏");

		final JLabel label_logo = new JLabel(ico);
		label_logo.setBounds(10, 10, 125, 50);
		separator_2.add(label_logo);//图片位
		...
		separator_3.add(button_default);//"恢复默认设置"框体		
	}
	...
	public ImageIcon getEf() {
		return ico;
	}

	public void setEf(ImageIcon ico) {
		this.ico = ico;
	}
