...
import priv.lt.snake.controller.Controller;
...
public class MainFrame extends JFrame implements GameListener {
	...
	public static void main(String args[]) {
		//异常处理
	try {
		MainFrame frame = new MainFrame(new Controller(new Snake(),new Food(), new Ground(), new Panel(), new JLabel()));
	...
	public MainFrame(Controller c) {
		...
		this.setTitle("贪吃蛇  by T.Liu");
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		this.setLayout(null);
		this.setResizable(false);//设置窗体是否可以由用户调整大小

		int left = 10;//框体宽度相关,这里只是先定义了这个常量，后面要用
		...
		optionPanel.getButton_default().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				gamePanel
						.setBackgroundColor(Panel.DEFAULT_BACKGROUND_COLOR);
